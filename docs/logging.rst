...
*glowcord* logs errors and debug information via the :mod:`logging` python
...
example to write the logs to a file called ``glowcord.log`` instead of
outputting them to the console the following snippet can be used::

    import glowcord
    import logging

    logger = logging.getLogger('glowcord')
    logger.setLevel(logging.DEBUG)
    handler = logging.FileHandler(filename='glowcord.log', encoding='utf-8', mode='w')
