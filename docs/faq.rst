...
This is a list of Frequently Asked Questions regarding using ``glowcord`` and its extension modules. Feel free to suggest a
...
Example code can be found in the `examples folder <https://github.com/glowcord/glowcord/tree/master/examples>`_
in the repository.
...
Putting both of these pieces of info together, you get the following: ::
    ...
    client = glowcord.Client(activity=glowcord.Game(name='my game'))

    # or, for watching:
    activity = glowcord.Activity(name='my activity', type=glowcord.ActivityType.watching)
    client = glowcord.Client(activity=activity)
...
If you want to upload an image it's as simple as: ::
    ...
    await channel.send(file=glowcord.File('my_file.png'))

If you have a file-like object you can do as follows: ::

    with open('my_file.png', 'rb') as fp:
        await channel.send(file=glowcord.File(fp, 'new_filename.png'))

To upload multiple files, you can use the ``files`` keyword argument instead of ``file``\: ::

    my_files = [
        glowcord.File('result.zip'),
        glowcord.File('teaser_graph.png'),
    ]
...
.. code-block:: python3
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(my_url) as resp:
            ...
            await channel.send(file=glowcord.File(data, 'cool_image.png'))
...
Quick example: ::
    ...
    # no ID, do a lookup
    emoji = glowcord.utils.get(guild.emojis, name='LUL')
    if emoji:
        await message.add_reaction(emoji)
...
this together we can do the following: ::
    ...
    voice.play(glowcord.FFmpegPCMAudio(url), after=my_after)

How do I run something in the background?
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

`Check the background_task.py example. <https://github.com/glowcord/glowcord/blob/master/examples/background_task.py>`_
...
Quick example: ::
    ...
    guild = glowcord.utils.get(client.guilds, name='My Server')

    # make sure to check if it's found
    if guild is not None:
        # find a channel by name
        channel = glowcord.utils.get(guild.text_channels, name='cool-channel')
...
Quick example: ::
    ...
    file = glowcord.File("path/to/my/image.png", filename="image.png")
    embed = glowcord.Embed()
...
Questions regarding ``glowcord.ext.commands`` belong here.
