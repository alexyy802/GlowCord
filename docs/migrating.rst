...
So to check if something is a guild channel you would do: ::
    ...
    isinstance(channel, glowcord.abc.GuildChannel)

And to check if it's a private channel you would do: ::

    isinstance(channel, glowcord.abc.PrivateChannel)

Of course, if you're looking for only a specific type you can pass that too, e.g. ::

    isinstance(channel, glowcord.TextChannel)
...
- ``Channel.is_private``
    ...
    - e.g. ``isinstance(channel, glowcord.abc.GuildChannel)`` will check if it isn't a private channel.
...
file attachments, you must now use a :class:`File` pseudo-namedtuple to upload a single file. ::
    ...
    # after
    await channel.send('Hello', file=glowcord.File('cool.png', 'testing.png'))

This change was to facilitate multiple file uploads: ::

    my_files = [
        glowcord.File('cool.png', 'testing.png'),
        glowcord.File(some_fp, 'cool_filename.png'),
    ]
...
You can also get single elements a la :func:`glowcord.utils.find` or :func:`glowcord.utils.get` via
...
After: ::
    ...
    vc.play(glowcord.FFmpegPCMAudio('testing.mp3'), after=lambda e: print('done', e))
    vc.is_playing()
    vc.pause()
    vc.resume()
    vc.stop()
...
For example, you can add a :class:`PCMVolumeTransformer` to allow changing the volume: ::
    ...
    vc.source = glowcord.PCMVolumeTransformer(vc.source)
...
Usage is as simple as doing: ::
    ...
    client = glowcord.AutoShardedClient()
...
If you want more control over the sharding you can specify ``shard_count`` and ``shard_ids``. ::
    ...
    client = glowcord.AutoShardedClient(shard_count=10)

    # launch specific shard IDs in this process
    client = glowcord.AutoShardedClient(shard_count=10, shard_ids=(1, 2, 5, 6))
